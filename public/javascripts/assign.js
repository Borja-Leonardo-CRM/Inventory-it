...
function createCells(response) {
  ...
  for (let i = 0; i < dataEquipment.length; i++) {
    if (dataEquipment[i].stock > 0) {
      assignedContent += `<li><a href="/equipments/${dataEquipment[i]._id}"> <b>Nombre :</b>  ${dataEquipment[i].name} <span class="stock-number"><b>Stock :</b>  ${dataEquipment[i].stock}</span><a><button type="button" id="assign-item" onclick="assign()"
